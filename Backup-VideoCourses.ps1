...
Function Backup-VideoCourses {
  ...
  for ($i = 0; $i -lt $numberOfVideos; $i++) {
    ...
    $courseFolder = (Join-Path -Path $targetDirectory -ChildPath $($filePath.Split('⌿')[0].Split([IO.Path]::GetInvalidFileNameChars()) -Join '')).Trim()
    $sectionFolder = (Join-Path -Path $courseFolder -ChildPath $($filePath.Split('⌿')[1].Split([IO.Path]::GetInvalidFileNameChars()) -Join '')).Trim()
    $videoFileName = (Join-Path -Path $sectionFolder -ChildPath $($filePath.Split('⌿')[2].Split([IO.Path]::GetInvalidFileNameChars()) -Join '')).Trim()
